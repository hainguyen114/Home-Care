...
CC_SRC_PATH="../cus_chains/"${CHAINCODE_NAME}"/"

rm ${CHAINCODE_NAME}.tar.gz
# import utils
. scripts/envVar.sh
...
chaincodeQuery() {
  ...
  # we either get a successful response, or reach MAX RETRY
    ...
    #CHAIN_LOWER_NAME=`echo "$CHANNEL_NAME" | tr [:upper:] [:lower:]`
    #peer chaincode query -C $CHANNEL_NAME -n ${CHAINCODE_NAME} -c '{"Args":["queryAllUsers"]}' >&log.txt
    res=$?
    set +x
		let rc=$res
		COUNTER=$(expr $COUNTER + 1)
